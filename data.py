...
class PreferenceSamplerConfig:
    ...
    samples_drawn_size: int = field(default=1000, metadata={'help': 'Size of preference pairs collected.'})
...
class PreferenceSampler:
    ...
    def generate_responses(self) -> dict:
        ...
        ref_model = LlamaForCausalLM.from_pretrained(self.model_path, device_map='cuda', torch_dtype=torch.bfloat16)
        ...
        # responses = {}
        # pbar_prompt_sample_indices = tqdm(prompt_sample_indices)
        # for prompt_idx in pbar_prompt_sample_indices:
        #     prompt = self.dataset[prompt_idx]
        #     pbar_prompt_sample_indices.set_description(f'Sampling prompt {prompt_idx + 1}')


        #     set_seed(data_config.generation_seed)
        #     out_1 = model_pipeline(prompt, generation_config=generate_config_policy_1)

        #     set_seed(data_config.generation_seed)
        #     out_2 = model_pipeline(prompt, generation_config=generate_config_policy_2)

        #     responses[prompt_idx] = {
        #         'response_1': out_1,
        #         'response_2': out_2
        #     }
